...
class ModelSelection:
    def __init__(self, data_path):
        ...
        self.models = {
            ...
            'Gradient Boosting' : AdaBoostClassifier(n_estimators=50),
    ...
    def load_data(self):
        try:
            logger.info("Loading cav file")
            df = pd.read_csv(self.data_path)
            df_sample = df.sample(frac=0.05, random_state=42)
            
            X = df_sample.drop(columns='satisfaction')
            y = df_sample['satisfaction']
            logger.info("Data loading and sampled successfully")
            return X,y
        
        except Exception as e:
            raise CustomException(f"Error while loading data", sys)
    ...
    def run(self):
        try:
            logger.info("Starting ModelSelecting pipeline")
            X,y = self.load_data()
